<template>
  <div class="swiper-cell">
    ...
    >
      <slot name="swiperCellContent"></slot>
    </div>
    <div class="swiper-cell-action">
      <slot name="swiperCellAction"></slot>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getWidth() {
      return new Promise((resolve, reject) => {
        ...
        query.select('.swiper-cell-action').boundingClientRect(data => {
          resolve(data.width)
        }).exec()
      })
    },
  ...
  mounted() {
    this.getWidth()
      .then(width => this.actionWidth = width) 
  }
}
</script>
